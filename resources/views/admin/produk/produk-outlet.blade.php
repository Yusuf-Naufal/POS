    ...
    <div class="w-full">
        ...
        <div class="flex justify-between flex-col md:flex-row items-center">
            <div class="ng-star-inserted open">
                <a href="{{ route('outlets.products.create', $outlet->id) }}" type="button" 
                    class="text-green-700 hover:text-white border border-green-700 hover:bg-green-800 focus:ring-4 focus:outline-none focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center me-2 mb-2 dark:border-green-500 dark:text-green-500 dark:hover:text-white dark:hover:bg-green-600 dark:focus:ring-green-800">
                    Tambah Produk
                </a>
            </div>
            <div class="flex items-center gap-4">
                <form method="GET" action="{{ route('outlets.products', $outlet->id) }}" class="flex items-center mt-4">
                    ...
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <a target="_blank" class="w-4 h-4">
                                <img class="w-20 h-20" src="{{ asset('public/assets/' . $produk->foto ) }}" alt="Produk">
                            </a>
                        </th>
                        <td class="px-6 py-4">
                            {{ $produk->nama_produk }}
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="relative flex items-center justify-center space-x-2 text-white bg-purple-700 hover:bg-purple-800 focus:ring-4 focus:ring-purple-300 font-medium rounded-lg text-sm px-5 py-2.5 mb-2 dark:bg-purple-600 dark:hover:bg-purple-700 dark:focus:ring-purple-900 w-fit">
                                <!-- Button for 'Ubah' -->
                                <a href="{{ route('outlets.products.edit', ['outlet' => $outlet->id, 'id' => $produk->id]) }}" type="button" 
                                ...
                                <div id="dropdown-{{ $index }}" class="dropdown-menu hidden absolute right-0 mt-2 w-48 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg">
                                    <ul class="py-1 text-sm text-gray-700 dark:text-gray-200">
                                        @if($produk->status === 'Aktif')
                                            <li>
                                                <form action="{{ route('outlets.products.deactivate', ['outlet' => $produk->id_outlet, 'id' => $produk->id]) }}" method="POST" class="block">
                                                    @csrf
                                                    @method('PATCH')
                                        ...
                                        @else
                                            <li>
                                                <form action="{{ route('outlets.products.activate', ['outlet' => $produk->id_outlet, 'id' => $produk->id]) }}" method="POST" class="block">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" class="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white w-full text-left">
                                                        Aktifkan
                                        ...
                                        @endif
                                        <li>
                                            <form id="delete-form-{{ $produk->id }}" action="{{ route('outlets.products.destroy', ['outlet' => $outlet->id, 'id' => $produk->id]) }}" method="POST" class="block">
                                                @csrf
                                                @method('DELETE')
